...
O repositório mantém as duas implementações da CLI para permitir uma
migração gradual:

- **Dart**, na raiz, é a implementação existente e continua sendo o fluxo
  padrão das releases `prt-*`;
- **Rust**, em [`rust/`](rust/), é a implementação nova com Ratatui e é
  distribuída nos artefatos `prt-rust-*`.

As duas implementações usam o mesmo nome de comando (`prt`) e a mesma
configuração em `~/.config/pr-tools`. Escolha explicitamente qual instalador
usar:
...
cargo clippy --manifest-path rust/Cargo.toml --locked --all-targets -- -D clippy::correctness
cargo test --manifest-path rust/Cargo.toml --locked
cargo build --manifest-path rust/Cargo.toml --locked --all-targets
```
...
`./rust/scripts/build.sh` gera `rust/dist/prt-rust-<plataforma>` para o host
atual, depois de executar as verificações Rust por padrão.
